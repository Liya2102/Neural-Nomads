...
def predict_class(sentence, model, words, classes):
    ...
    if top_prob.item() < 0.75:  # confidence threshold
        return None
...
# Load saved model + words + classes
...
with open("alumni_chatbot.json") as f:
    intents = json.load(f)
...
if __name__ == "__main__":
    while True:
        ...
        if msg.lower() in ["quit", "exit", "bye"]:
            print("Bot: Goodbye!")
            break
        res = get_response(msg, model, words, classes, intents)
        print("Bot:", res)